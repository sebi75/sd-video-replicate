...
def get_batch(keys, value_dict, N: Union[List, ListConfig], device="cuda"):
    ...
    for key in keys:
        if key == "txt":
            ...
            batch_uc["txt"] = (
                np.repeat([value_dict["negative_prompt"]],
                          repeats=math.prod(N))
                .reshape(N)
                .tolist()
            )
            ...
            batch["aesthetic_score"] = (
                torch.tensor([value_dict["aesthetic_score"]]).to(
                    device).repeat(*N, 1)
            )
...
def get_input_image_tensor(image: Image.Image, device="cuda"):
    ...
    image_tensor = torch.from_numpy(image_array).to(
    ...
    with torch.no_grad():
        with autocast(device) as precision_scope:
            with model.ema_scope():
                ...
                for k in c:
                    c[k], uc[k] = map(
                        lambda y: y[k][:num_samples].to(device), (c, uc))
